...
typedef struct {          // Структура тестов:
	...
	const char* output;
	size_t max_length;               // Максимальная длина строки
...
int main(void) {
	...
	dataSet tests[] = {
		{ "input1.txt", "output1.txt", 50, SUCCESS },
		{ "input2.txt", "output2.txt", 50, SUCCESS },
		{ "input3.txt", "output3.txt", 50, SUCCESS }
	};
	...
	for (int i = 0; i < test_num; i++) {                                            // Тестирование
		...
		split_long_lines(input, output, tests[i].max_length, & err);
