...
float tochka (FILE *f1, float x){
    ...
    int flag = 0 ;
    while(fscanf (f1, "%f", &chislo) == 1){ //цикл считывающий все числа из последовательности
...
float proizvodnaya (FILE *f2, float y){
    ...
    int flag = 0;
    while(fscanf(f2, "%f", &chislo) == 1){ //цикл считывающий все числа из последовательности
            if(flag >= 1){
