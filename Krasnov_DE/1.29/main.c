...
float tochka (FILE *f1, float x){
    ...
    int flag = 0 ;
    int a;
    while((a = fscanf (f1, "%f", &chislo)) != EOF){
        if(a != 1){
            ERROR = -1;
            return -1;
        }
...
float proizvodnaya (FILE *f2, float y){
    ...
    int poryadok_element = 0;
    int a;
    while((a=fscanf(f2, "%f", &chislo)) != EOF){
        if(a != 1){
            ERROR=-1;
            return -1;
        }
        if(poryadok_element >= 1){
            proizvodnay_v_tochke = proizvodnay_v_tochke + chislo * stepen_y * poryadok_element;
            stepen_y = stepen_y * y;
        }
        poryadok_element = poryadok_element + 1;
