...
{ 
    double x, next_x;
	double eps = 0.00001;  // задали определённую точность для вещественных чисел
    
    
    if (fscanf(file,"%lf",&x)!=1){   //прочитали первое число и вывели ошибку при пустом файле
		return -1;
	}

    while (fscanf(file, "%lf", &next_x) == 1) {  // пока мы можем считать следующее число, проверяем их равенство
...
int main(void) {
    ...
    int f;
    FILE *file = fopen("input.txt", "r");  //открыли файл для чтения
    f = is_equal(file);
    ...
    else if (f == -1)       // иначе печатаем, что файл пустой
    {
        printf("error, pusto\n");
    }
	
    fclose(file);  // закрыли файл, который читали
