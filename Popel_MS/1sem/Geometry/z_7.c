...
#include <math.h>
#include <stdbool.h>
			//Необходимо найти рассточние между двумя многоугольниками, заданными координатами точек. Если мн-ки пересекаются - расстояние равно 0.
typedef struct
{
	double x;
	double y;
...
void to_gnuplot(FILE *gnuplt); //Загрузка координат многоугольника в файл для постройки графика
double scalar_product(Point a, Point b, Point c, Point d); //скалярное произведение
double oblique_product(Point a, Point b, Point c, Point d); //косое произведение
Segm point_distance(Point a, Point b); //расстояние между точками
Segm segment_point_distance(Point a, Point b, Point c); //расстояние между точкой и отрезком
bool is_cross(Point p1, Point p2, Point m1, Point m2); //проверка пересечения многоугольников
Segm segment_distance(Point a, Point b, Point c, Point d); //рассчет минимального расстояния
